...
const CURRENCY_FORMAT: Record<Currency, (v: number) => string> = {
  btc: (v) => `${Number(v).toFixed(8)} BTC`,
  usd: (v) => `$${Number(v).toFixed(2)}`,
  eur: (v) => `€${Number(v).toFixed(2)}`,
  jpy: (v) => `¥${Number(v).toFixed(0)}`,
  gbp: (v) => `£${Number(v).toFixed(2)}`,
  cad: (v) => `CA$${Number(v).toFixed(2)}`,
  aud: (v) => `A$${Number(v).toFixed(2)}`,
  chf: (v) => `CHF ${Number(v).toFixed(2)}`,
  inr: (v) => `₹${Number(v).toFixed(0)}`,
  krw: (v) => `₩${Number(v).toFixed(0)}`,
  brl: (v) => `R$${Number(v).toFixed(2)}`,
  hkd: (v) => `HK$${Number(v).toFixed(2)}`,
  sgd: (v) => `S$${Number(v).toFixed(2)}`,
};
...
const MempoolLatencyStats = () => {
  ...
  const formatTotalFee = (currency: Currency): string => {
    ...
    const amount =
      currency === 'btc'
        ? m.total_fee_btc
        : (m as unknown as Record<string, number>)[`total_fee_${currency}`] ??
          0;
  ...
  const formatFee = (fee: Fee | undefined, currency: Currency): string => {
    ...
    const amount =
      currency === 'btc'
        ? fee.fee_btc
        : (fee as unknown as Record<string, number>)[`fee_${currency}`] ?? 0;
